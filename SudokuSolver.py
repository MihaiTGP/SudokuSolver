def find_next_empty(grid):
    """this function finds the next empty cell in the given grid"""
    for row in range(9):
        for col in range(9):
            if grid[row][col] == '':
                return row, col
    # if it can't find empty cells the puzzle is solved/almost solved and doesn't return a row and a col
    return None, None

def is_valid(grid, guess, row, col):
    """checks if the numbers respect the rules of the game"""
    row_vals = grid[row]
...
def string_to_integer(grid):
    """transform all the elements in the grid to integers"""
    for row in range(9):
        for col in range(9):
            if grid[row][col] != '':
...
def integer_to_string(grid):
    """transform all the elements in the grid back to strings"""
    for row in range(9):
        for col in range(9):
